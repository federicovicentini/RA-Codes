...
# Step 1: Calculate cr4 and cr20 with naics4 sector sales
compustat <- compustat %>%
  group_by(fyear) %>%
  ...
  mutate(naics4cr4 = sum(ifelse(rank4<=4, sale, 0))/sector_sales) %>%
  mutate(naics4cr20 = sum(ifelse(rank4<=20, sale, 0))/sector_sales)

# Step 1: Calculate cr4 and cr20 with naics3 sector sales
compustat <- compustat %>%
  ...
  mutate(naics3cr4 = sum(ifelse(rank3<=4, sale, 0))/sector_sales3) %>%
  mutate(naics3cr20 = sum(ifelse(rank3<=20, sale, 0))/sector_sales3)
  
# Aggregate database with cr4 by naics2 with naics4 aggr
aggcr <- compustat %>%
  group_by(fyear, naics3) %>%
  summarize(cr4 = cr4,cr20=cr20, naics1=naics1, naics2 = naics2, sectorweight = sectorweight4, sector = sector) %>%
  group_by(fyear, naics2) %>%
  summarize(cr4 = weighted.mean(cr4, sectorweight), sector = sector, 
            cr20 = weighted.mean(cr20, sectorweight))
...
ggplot(aggcr, aes(x = fyear, y = cr4, color = naics2)) +
  geom_line() +
...
ggplot(aggcr, aes(x = fyear, y = cr20, color = naics2)) +
  geom_line() +
  facet_wrap(~ naics2, scales = "free_y") +
