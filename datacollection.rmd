...
compustat <- compustat %>%
  ...
  mutate(Autorweight = Autorsales/total_sales)
...
```{r output}
output <- data.frame(
  Variable = c("GDP growth", "Concentration", "Markup", "Labor share"),
  `E.1980s` = c(mean80s, 38, avg80, labsh6mean1),
  `E.2010s` = c(mean10s, 43.5, avg10, labsh6mean2),
  `E.Delta` = c(mean10s-mean80s, 5.5, avg10-avg80, labsh6mean2-labsh6mean1),
  `S.1980s` = c(3.97, 7.73, 1.33, 77.96),
  `S.2010s` = c(2.28, 17.27, 1.47, 74.14),
  `S.Delta` = c(-1.69, 9.54, 0.15, -3.82)
)

# Approximate values to 2 significant digits
output <- lapply(output, signif, digits = 3)

# Create the table and apply styling
styled_table <- kable(output, format = "latex", caption = "Empirical and Simulated Data") %>%
  kable_styling(
