...
compustat <- compustat %>%
  ...
  mutate(sectorweight = sector_sales/total_sales)%>%
  mutate(rank = dense_rank(desc(sale))) %>%
  mutate(population = n()) %>%
  mutate(cr4 = sum(ifelse(rank<=4, sale, 0))/sector_sales) %>%
  mutate(cr20 = sum(ifelse(rank<=20, sale, 0))/sector_sales)
  
  
aggcr <- compustat %>%
  group_by(fyear, naics3) %>%
  summarize(cr4 = cr4,cr20=cr20, naics1=naics1, naics2 = naics2, sectorweight = sectorweight, sector = sector) %>%
