...
# Step 1: Calculate cr4 and cr20 with naics4 sector sales and empl
compustat <- compustat %>%
  group_by(fyear) %>%
...
aggcr4 <- compustat %>%
  ...
  summarize(cr4sal = n4cr4sal,cr20sal=n4cr20sal, cr4emp=n4cr4emp, cr20emp=n4cr20emp,
            naics1=naics1, naics2 = naics2, sectorweight = sectorweight4, sector = sector) %>%
  ...
  summarize(cr4sal = weighted.mean(cr4sal, sectorweight), sector = sector, 
            ...
            cr4emp = weighted.mean(cr4emp, sectorweight),
            cr20emp = weighted.mean(cr20emp, sectorweight))
...
ggplot(aggcr4, aes(x = fyear, color = naics2)) +
  geom_line(aes(y = cr4sal)) +
  geom_line(aes(y = cr4emp), color="black") +
...
ggplot(aggcr4, aes(x = fyear, color = naics2)) +
  geom_line(aes(y = cr20sal)) +
  geom_line(aes(y = cr20emp), color="black") +
  facet_wrap(~ naics2, scales = "free_y") +
  labs(x = "Year", y = "CR20", title = "Time Series of CR20 by Sector") +
  theme_minimal() +
  guides(color = "none")
