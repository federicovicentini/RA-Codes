...
packages <- c("eurostat", "stargazer", "fredr", "kableExtra",
              ...
              "wbstats", "Rilostat", "sandwich", "dplyr")
...
findconversion = function(s1,s2){
    model <- lm(s1 ~ lag(s1, n=1L))
    # Check coefficient of lagged variable
    lag_coeff <- coef(model)["lag_ts1"]
    # Check if first lag residuals are zero
    if (lag_coeff == 0) {
        cat("The first-lag residuals of ts1 are zero.\n")
    } else {
        cat("The first-lag residuals of ts1 are not zero.\n")
    }
    model <- lm(s2 ~ lag(s2, n=1L))
    # Check coefficient of lagged variable
    lag_coeff <- coef(model)["lag_ts2"]
    # Check if first lag residuals are zero
    if (lag_coeff == 0) {
        cat("The first-lag residuals of ts1 are zero.\n")
    } else {
    ...
    s2=s2-lag(s2, n=1L)
    train = lm(s1 ~ s2)
    stargazer(train, type = "text", robust = TRUE)
